...
接触角と接触半径は、選択中の密度フィールドでしきい値以上のセル中心点を液滴点群として使い、`Sesshoku6.py` と同じ球フィット式で計算します。xy方向の周期境界をまたぐ液滴に対応するため、既定では球フィット前にxy座標を周期補正します。
...
- `mdfoam_summary.csv`: ケースごとの最大体積、最終体積、蒸発完了時刻、初期/最終有効の接触角・接触半径
